...
begin
  ...
  begin
    if Pos(C_ARG_WOT_PATH_WIN32, ParamStr(i)) > 0 then
      Config.WOTPath := Trim(Copy(ParamStr(i), Length(C_ARG_WOT_PATH_WIN32)+1, MaxInt));
    if Pos(C_ARG_SILENT_MODE_WIN32, ParamStr(i)) > 0 then
      Config.SilentMode := True;
    if Pos(C_ARG_CREATE_BACKUP_WIN32, ParamStr(i)) > 0 then
      Config.CreateBackup := False;
    if Pos(C_ARG_ADD_MARK_WIN32, ParamStr(i)) > 0 then
      Config.AddMark := False;
  end;
end;
...
type
    ...
    try
      ...
      try
        ...
        begin
          ...
          FS.ReadBuffer(Pointer(Result)^, Len);
      ...
      on E: Exception do
      begin
        WriteLn('Error in GetFileMark: ' + E.Message);
        Result := '';
      end;
    end;
  end;
  ...
  var
    SignsTextList, ValueList: TStringList;
  ...
  begin
        ...
        for j := 1 to Length(Str) do
          ...
          begin
            ...
            case Str[j] of
              ...
              ValueList.Strings[k] := ValueList.Strings[k] + Str[j];
            end;
    ...
    begin
      ...
      while i < Length(SignsList) do
        try
          if SignsList[i].XmlVersion.Count = 0 then
            Continue;
          ...
          begin
            if i <> 0 then
              Result := SignsList[i - 1];
            Exit;
          end;
          if SignsList[i].ExeVersion.Count = 0 then
            Continue;
          ...
          if SignsList[i].ExeVersion.ID = 0 then
            Continue;
          if SignsList[i].ExeVersion.ID < ExeFileVersion.ID then
            Continue
...
begin
  ...
  InitConfig;
  ParseArguments;
  if Length(Config.WOTPath) > 0 then
  begin
    if Config.WOTPath[Length(Config.WOTPath)] <> '\' then
      Config.WOTPath := Config.WOTPath + '\';
    if not DirectoryExists(Config.WOTPath) then
      Config.WOTPath := '';
  end;
  if Length(Config.WOTPath) = 0 then
    Config.WOTPath := ExtractFilePath(ParamStr(0));
  Write(C_H_PREFIX + 'Game client exe-file search... ');
  Str := LowerCase(ExtractFileName(LeftStr(Config.WOTPath, Length(Config.WOTPath) - 1)));
  ...
  begin
    ...
    {$ENDIF}
    if not Config.SilentMode then Readln;
    Halt(ERROR_XBIT);
  end;
  if Pos({$IFDEF WOT32}C_WOT32EXE_FOLDER{$ELSE}C_WOT64EXE_FOLDER{$ENDIF}, Str) > 0 then
  begin
    ExeFileName := Config.WOTPath + C_WOTEXE_FILENAME;
    if not FileExists(ExeFileName) then
      ExeFileName := Config.WOTPath + C_LMTEXE_FILENAME;
    if FileExists(ExeFileName) then
      Config.WOTPath := ExtractFilePath(ExtractFileDir(ExeFileName))
    else
      ExeFileName := '';
  end
  else if DirectoryExists(Config.WOTPath + {$IFDEF WOT32}C_WOT32EXE_FOLDER{$ELSE}C_WOT64EXE_FOLDER{$ENDIF}) then
  begin
    Str := Config.WOTPath + {$IFDEF WOT32}C_WOT32EXE_FOLDER{$ELSE}C_WOT64EXE_FOLDER{$ENDIF} + '\';
  ...
  Writeln('');
  XMLFileName := Config.WOTPath + 'version.xml';
  ...
  begin
    ...
    if XmlVersion.Count > 0 then
      ...
      Writeln(C_L_PREFIX + 'Version: not decoded!');
  end
  else
    Writeln(C_L_PREFIX + 'Version: not found!');
  Writeln('');
  ...
  if SameText(ExtractFileName(ExeFileName), C_LMTEXE_FILENAME) then
    ...
    Writeln(C_L_PREFIX + 'FileVendor:     Wargaming.net');
  ...
  if ExeFileVersion.Count > 0 then
    ...
    Writeln(C_L_PREFIX + 'FileVersion:    not decoded!');
  ...
  if ExeProductVersion.Count > 0 then
    ...
    Writeln(C_L_PREFIX + 'ProductVersion: not decoded!');
  ...
  begin
    ...
    if Length(Sign.ExeVersion.Text) > 0 then
      if Sign.ExeVersion.ID > 0 then
        ...
        Writeln(C_L_PREFIX + 'FileVersion:   ' + Sign.ExeVersion.Text)
    else
      Writeln(C_L_PREFIX + 'FileVersion:   -');
  end
  else
  begin
    Writeln('not found!');
